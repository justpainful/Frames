...
struct DocumentCodingTests {
    ...
    private func expectRoundTrip(_ document: EditDocument, _ decoded: EditDocument) {
        #expect(decoded.schemaVersion == document.schemaVersion, "schemaVersion")
        #expect(decoded.id == document.id, "id")
        #expect(decoded.kind == document.kind, "kind")
        #expect(decoded.assets == document.assets, "assets")
        #expect(decoded.videoTrack == document.videoTrack, "videoTrack")
        #expect(decoded.photo == document.photo, "photo")
        #expect(decoded.audioClips == document.audioClips, "audioClips")
        #expect(decoded.textOverlays == document.textOverlays, "textOverlays")
        #expect(decoded.imageOverlays == document.imageOverlays, "imageOverlays")
        #expect(decoded.drawings == document.drawings, "drawings")
        #expect(decoded.blurRegions == document.blurRegions, "blurRegions")
        #expect(decoded.selectiveAdjustments == document.selectiveAdjustments, "selectiveAdjustments")
        #expect(decoded.effects == document.effects, "effects")
        #expect(decoded.trackedObjects == document.trackedObjects, "trackedObjects")
        #expect(decoded.grade == document.grade, "grade")
        #expect(decoded.portrait == document.portrait, "portrait")
        #expect(decoded.background == document.background, "background")
        #expect(decoded.outputAspect == document.outputAspect, "outputAspect")
        #expect(decoded.audioMix == document.audioMix, "audioMix")
        #expect(decoded.safeAreaGuides == document.safeAreaGuides, "safeAreaGuides")
        ...
        #expect(abs(decoded.createdAt.timeIntervalSince(document.createdAt)) < 0.001, "createdAt")
        #expect(abs(decoded.modifiedAt.timeIntervalSince(document.modifiedAt)) < 0.001, "modifiedAt")
    }
